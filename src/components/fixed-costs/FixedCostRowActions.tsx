...
import {
  deleteFixedCostItemAction,
  setFixedCostItemActiveAction,
...
export function FixedCostRowActions({
  ...
  function toggleActive() {
    startTransition(async () => {
      const res = await setFixedCostItemActiveAction(id, !active);
  ...
  function remove() {
    if (
      !window.confirm(
        "Excluir este item de custo fixo? O total mensal será recalculado.",
      )
    )
      return;
    startTransition(async () => {
      const res = await deleteFixedCostItemAction(id);
      if (!res.ok) window.alert(res.error);
      else router.refresh();
    });
  }
